...
module.exports.register = (app) => {
  app.get(`/${config.apiPrefix}/v1/loading`, (req, res) => {
    if (utils.authenticate(req.headers.authorization)) {
      const worker = new Worker(path.join(__dirname, '..', '..', '..', 'workers', 'v1', 'systeminformation', 'loading', 'current.js'));
  ...
  app.get(`/${config.apiPrefix}/v1/loading/full`, (req, res) => {
    if (utils.authenticate(req.headers.authorization)) {
      const worker = new Worker(path.join(__dirname, '..', '..', '..', 'workers', 'v1', 'systeminformation', 'loading', 'full.js'));
      utils.registerWorker(res, worker);
    } else {
      utils.badRequest(req, "❌ 認證失敗");
    }
  ...
  app.get(`/${config.apiPrefix}/v1/loading/process/:process_name`, (req, res) => {
    if (utils.authenticate(req.headers.authorization)) {
      const worker = new Worker(path.join(__dirname, '..', '..', '..', 'workers', 'v1', 'systeminformation', 'loading', 'process.js'));
      utils.registerWorker(res, worker, {
  ...
  app.get(`/${config.apiPrefix}/v1/loading/service/:service_name`, (req, res) => {
    if (utils.authenticate(req.headers.authorization)) {
      const worker = new Worker(path.join(__dirname, '..', '..', '..', 'workers', 'v1', 'systeminformation', 'loading', 'service.js'));
      utils.registerWorker(res, worker, {
